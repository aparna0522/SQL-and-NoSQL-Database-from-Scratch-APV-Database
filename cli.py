...
sql_actions = {
    "load": ["load data in <table_name> with <path> generate PrimaryKey provided headers", 
             "\t-<table_name> is the table name you want to load data in", 
             "\t-<path> is the absolute path to the csv file you wish to load"
             "\t-'generate PrimaryKey' add this if you wish to append row number at start of each entry",
...
def main():   
    ...
    while True:
        #user_input = input("{}".format(intro)).strip()
        user_input = ""
        line = ""
        prints = intro
        while True:
...
def sql_process_user_command(command):
    ...
    if action == "load":
        ...
        a_format_entry = sql_merge_query_retriever(a_statement)
        b_format_entry = sql_merge_query_retriever(b_statement)
...
def nosql_process_user_command(command):
    ...
    if action == "load":
        ...
        a_statement = re.findall(r'\(.*?\)', command)[0][1:-1]
        b_statement = re.findall(r'\(.*?\)', command)[1][1:-1]
        a_params = join_params_retrieval(a_statement)
        b_params = join_params_retrieval(b_statement)
